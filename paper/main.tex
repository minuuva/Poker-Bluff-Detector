...
camera-geometry offsets. Within sessions, by contrast, round 1 found suggestive
signal (pooled cross-validated AUC $0.574$, CI $[0.492, 0.652]$). In a second
preregistered round on an enlarged corpus of eight sessions (46.6 hours,
9{,}801 labeled decisions, 494 camera-covered), the within-session
question resolved positive: pooled within-session behavior-only AUC is
$0.587$ (95\% CI $[0.515, 0.655]$), excluding chance under the frozen
criterion, while cross-session transfer stays at or below chance
($0.430$, $[0.358, 0.502]$). A preregistered frozen DINOv2 embedding
probe adds nothing over betting action out of session ($+0.012$,
$[-0.079, +0.103]$), extending the conclusion to learned
representations. Tells therefore exist and are machine-readable within a
session, and do not transfer across sessions. Decision timing, which
needs no camera, is bounded below $+0.01$ AUC over betting action. We conclude that identity hygiene, not
...
session). The first four sessions constitute round 1; four further
sessions were added under the round 2 preregistration. The stream exposes hole cards in its graphics, providing ground
...
the within-session estimate, and a frozen DINOv2 ViT-S/14 probe
(identity-gated face and pose crops, mean and std pooling, PCA to 32
components fit on training folds) to test whether a learned
representation changes the out-of-session answer.

...
\caption{One annotated frame from the released demo renderer (October
2025 session, hand 120, river decision). Green box: identity-gated face
attribution with its re-identification score. Colored contours are
MediaPipe face landmarks (brows, eyelids, lips); the magenta ray is the
...
features; the tag beside the box is a live event detector (brow
furrowed). Top right: the held-out six-class strength distribution, the
...
decision clock from the OCR game state. The gold player panels and pot
graphics are part of the source broadcast. This session is the one in
which the primary player wore no eyewear, so eyelid and gaze tracking
are unoccluded.}
\label{fig:demo}
\end{figure*}
